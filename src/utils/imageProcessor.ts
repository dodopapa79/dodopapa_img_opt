...
export async function processAndOptimizeImage(
  ...
  const resizeMode = settings.resizeMode || (settings.maxWidthOption === 'original' ? 'original' : 'width');
  const targetDim = settings.targetDimension || (settings.maxWidthOption !== 'original' ? parseInt(settings.maxWidthOption, 10) : null);

  // Check if image width is smaller than the configured target width and padding is requested (only applies when mode is width)
  const isWidthBelowTarget = Boolean(
    resizeMode === 'width' &&
    targetDim &&
    settings.padSmallImages &&
    img.naturalWidth < targetDim
  );
  ...
  if (isWidthBelowTarget && targetDim) {
    usePadding = true;
    canvasW = targetDim;
    ...
    // Calculate destination dimensions proportionally based on resizeMode
    let targetW = sourceW;
    let targetH = sourceH;

    if (resizeMode === 'width' && targetDim && targetW > targetDim) {
      // 가로 기준 비율 축소 (세로는 비율대로 자동 계산)
      const scale = targetDim / targetW;
      targetW = targetDim;
      targetH = Math.round(targetH * scale);
    } else if (resizeMode === 'height' && targetDim && targetH > targetDim) {
      // 세로 기준 비율 축소 (가로는 비율대로 자동 계산)
      const scale = targetDim / targetH;
      targetH = targetDim;
