...
export type ResizeMode = 'width' | 'height' | 'original';
...
export interface OptimizationSettings {
  ...
  maxWidthOption: string; // 'original' | '1200' | '900' | '860' | '600' | 'custom' 등
  resizeMode: ResizeMode; // 'width' (가로 기준 비율축소) | 'height' (세로 기준 비율축소) | 'original' (원본 유지)
  targetDimension: number; // 목표 치수 (px)
