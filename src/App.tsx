  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8F8FA] text-[#18181B] selection:bg-black selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        {/* Main Tab Navigation */}
        <div className="flex items-center gap-2 mb-6 border-b border-zinc-200 pb-3 overflow-x-auto no-scrollbar">
          <button
            type="button"
          ...
          >
            <Zap className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'optimizer' ? 'text-amber-400' : 'text-zinc-400'}`} />
            <span>이미지 일괄 최적화</span>
            {images.length > 0 && (
