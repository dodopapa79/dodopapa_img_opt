...
export function ThumbnailMaker() {
  ...
  return (
    <div className="bg-white rounded-2xl border border-zinc-200/90 shadow-sm overflow-hidden mb-8">
      {/* Header Bar */}
      <div className="px-3 sm:px-5 py-3 sm:py-4 border-b border-zinc-200 bg-zinc-50/70 flex flex-wrap items-center justify-between gap-2.5">
        <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-black text-white flex items-center justify-center shadow-xs shrink-0">
            <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-400" />
          </div>
          <div className="min-w-0">
            <h2 className="text-sm sm:text-base font-bold text-zinc-900 flex items-center gap-1.5 truncate">
              <span>블로그 썸네일 제작기</span>
              <span className="text-[10px] sm:text-xs px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-semibold whitespace-nowrap">
                스마트 스튜디오
              </span>
            </h2>
            <p className="text-[11px] sm:text-xs text-zinc-500 hidden sm:block truncate">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap shrink-0">
          <div className="flex items-center gap-1 bg-zinc-200/60 p-0.5 sm:p-1 rounded-xl">
            {[
              { id: '1:1' as const, label: '1:1', full: '정사각형 (1:1)', sub: '1080×1080' },
              { id: '16:9' as const, label: '16:9', full: '와이드 (16:9)', sub: '1280×720' },
              { id: '4:3' as const, label: '4:3', full: '클래식 (4:3)', sub: '1200×900' },
            ].map((r) => (
              <button
                key={r.id}
                type="button"
                onClick={() => handleRatioChange(r.id)}
                className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-semibold transition-all ${
                  ratio === r.id
              ...
              >
                ...
                <div className="text-[9px] sm:text-[10px] font-mono text-zinc-400 font-normal">{r.sub}</div>
              </button>
            ))}
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl border border-zinc-300 bg-white hover:bg-rose-50 hover:border-rose-200 hover:text-rose-600 text-zinc-700 text-[11px] sm:text-xs font-semibold shadow-2xs transition-all active:scale-95 cursor-pointer"
            title="모든 설정을 초기 기본 상태로 되돌립니다"
          >
            <RotateCcw className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-zinc-500 hover:text-rose-500" />
            <span>설정 초기화</span>
          </button>
        </div>
      </div>
